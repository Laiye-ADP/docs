...
Method 2: Query the full list of your applications via API. [API for querying application list](../api-reference/adp-api/application-management/list-applications)
...
## Check both the HTTP status and the business result

An HTTP 2xx status only confirms that the server received the request; it does not always mean that the business operation succeeded. Check the HTTP status first, then parse the JSON response and inspect the top-level `code`. If `code` is not the success value documented by the endpoint, handle the error using `message` and `tips`. For document extraction and parsing, also inspect `data.status` before consuming the result.

The Python samples on the relevant endpoint pages demonstrate this pattern. HTTP 422 request validation errors use FastAPI's `detail[]` structure, which is different from a business error returned with HTTP 200.
...
<Warning>
  Only workflow application details and run export are currently marked **Limited preview** and are intended for evaluation and controlled integrations. Their contracts, permission boundaries, and availability may change and must not be treated as a production SLA or critical-path dependency.
</Warning>
